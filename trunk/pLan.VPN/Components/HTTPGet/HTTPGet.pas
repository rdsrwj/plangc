...
type
  TOnProgressEvent = procedure(Sender: TObject; TotalSize,
    Readed: Integer) of object;

  TOnDoneFileEvent = procedure(Sender: TObject; FileName: String;
  ...
  private
    FTAcceptTypes: String;
    FTAgent: String;
    FTURL: String;
    FTFileName: String;
    FTStringResult: String;
    FTUserName: String;
    FTPassword: String;
    FTPostQuery: String;
    FTReferer: String;
  ...
  public
    constructor Create(AAcceptTypes, AAgent, AURL, AFileName, AUserName,
      APassword, APostQuery, AReferer: String; ABinaryData, AUseCache: Boolean;
...
begin
  try
    ...
    if (FTAgent <> '') then
      hSession := InternetOpen(PChar(FTAgent), INTERNET_OPEN_TYPE_DIRECT,
        nil, nil, 0)
    else
    ...
    FTResult := HttpQueryInfo(hRequest, HTTP_QUERY_CONTENT_LENGTH, Buf,
      dwBufLen, dwIndex);
...
begin
  ...
  begin
    FThread := THTTPGetThread.Create(FAcceptTypes, FAgent, FURL, FFileName,
      FUserName, FPassword, FPostQuery, FReferer, FBinaryData, FUseCache,
      FProgress, True);
...
begin
  ...
  RegisterComponents('pLan Components', [THTTPGet]);
end;
