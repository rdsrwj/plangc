...
uses
  ShellAPI, Windows,
  {$IFDEF RX_D3} ActiveX, ComObj {$ELSE} Ole2, OleAuto {$ENDIF},
...
type
  ...
  private
    RichEdit: TIRCCustomRichEdit;
    FType: TIRCAttributeType;
  ...
  {$ENDIF}
    ...
    function GetConsistentAttributes: TIRCConsistentAttributes;
  ...
  public
    constructor Create(AOwner: TIRCCustomRichEdit;
      AttributeType: TIRCAttributeType);
  ...
  {$ENDIF}
    ...
    property ConsistentAttributes: TIRCConsistentAttributes
    ...
    property RevAuthorIndex: Byte read GetRevAuthorIndex
      write SetRevAuthorIndex;
    property SubscriptStyle: TSubscriptStyle read GetSubscriptStyle
      write SetSubscriptStyle;
    property Size: Integer read GetSize write SetSize;
    property Style: TFontStyles read GetStyle write SetStyle;
    property Height: Integer read GetHeight write SetHeight;
    property UnderlineType: TUnderlineType read GetUnderlineType
      write SetUnderlineType;
  end;
  ...
  private
    ...
    function GetNumbering: TIRCNumbering;
    function GetNumberingStyle: TIRCNumberingStyle;
    ...
    procedure SetNumbering(Value: TIRCNumbering);
    procedure SetNumberingStyle(Value: TIRCNumberingStyle);
  ...
  public
    ...
    property HeadingStyle: THeadingStyle read GetHeadingStyle
      write SetHeadingStyle;
    ...
    property Numbering: TIRCNumbering read GetNumbering write SetNumbering;
    property NumberingStyle: TIRCNumberingStyle read GetNumberingStyle
      write SetNumberingStyle;
...
{$ENDIF}
  ...
  TIRCCustomRichEdit = class(TCustomMemo)
  private
    ...
    FPlainText: Boolean;
    FSelAttributes: TIRCTextAttributes;
    FDefAttributes: TIRCTextAttributes;
    FWordAttributes: TIRCTextAttributes;
    FParagraph: TIRCParaAttributes;
  ...
  {$ENDIF}
    ...
    procedure SetDefAttributes(Value: TIRCTextAttributes);
    procedure SetSelAttributes(Value: TIRCTextAttributes);
    procedure SetWordAttributes(Value: TIRCTextAttributes);
    ...
    procedure SetupFindDialog(Dialog: TFindDialog; const SearchStr,
      ReplaceStr: String);
    function FindEditText(Dialog: TFindDialog; AdjustPos,
  ...
  {$IFDEF RX_D3}
    ...
    procedure CMDocWindowActivate(var Message: TMessage);
      message CM_DOCWINDOWACTIVATE;
  ...
  {$IFDEF RX_D3}
    ...
    property AllowInPlace: Boolean read FAllowInPlace write FAllowInPlace
      default True;
  {$ENDIF}
    property AllowObjects: Boolean read FAllowObjects write SetAllowObjects
      default True;
    ...
    property LangOptions: TRichLangOptions read GetLangOptions
      write SetLangOptions default [rlAutoFont];
    ...
    property SelectionBar: Boolean read FSelectionBar write SetSelectionBar
      default True;
    property StreamFormat: TRichStreamFormat read GetStreamFormat
      write SetStreamFormat default sfDefault;
    property StreamMode: TRichStreamModes read GetStreamMode
      write SetStreamMode default [];
    property UndoLimit: Integer read FUndoLimit write SetUndoLimit default 100;
    property WordSelection: Boolean read GetWordSelection
      write SetWordSelection default True;
    ...
    property OnURLClick: TRichEditURLClickEvent read FOnURLClick
      write FOnURLClick;
    property OnTextNotFound: TRichEditFindErrorEvent read FOnTextNotFound
      write FOnTextNotFound;
  {$IFDEF RX_D3}
  ...
  {$ENDIF}
    ...
    property DefAttributes: TIRCTextAttributes read FDefAttributes
      write SetDefAttributes;
    property SelAttributes: TIRCTextAttributes read FSelAttributes
      write SetSelAttributes;
    property WordAttributes: TIRCTextAttributes read FWordAttributes
      write SetWordAttributes;
    property PageRect: TRect read FPageRect write FPageRect;
    property Paragraph: TIRCParaAttributes read FParagraph;
    property SelectionType: TRichSelectionType read GetSelectionType;
  end;

  { TIRCRichEdit }

  TIRCRichEdit = class(TIRCCustomRichEdit)
  private
    FIRCRichEditBusy: Boolean;
    FSmiles: TImageList;
    procedure SetSmiles(Value: TImageList);
  protected
...
function BitmapToRTF(pict: TBitmap; var _rtf: AnsiString): AnsiString;
var
  bi, bb, rtf: String;
  bis, bbs: Cardinal;
  achar: ShortString;
...
const
  AttrFlags: array[TIRCAttributeType] of Word = (0, SCF_SELECTION,
    SCF_WORD or SCF_SELECTION);

constructor TIRCTextAttributes.Create(AOwner: TIRCCustomRichEdit;
  AttributeType: TIRCAttributeType);
begin
  inherited Create;
  RichEdit := AOwner;
...
procedure TIRCTextAttributes.InitFormat(var Format: TCharFormat2);
begin
...
function TIRCTextAttributes.GetConsistentAttributes: TIRCConsistentAttributes;
var
  Format: TCharFormat2;
begin
  Result := [];
...
procedure TIRCTextAttributes.GetAttributes(var Format: TCharFormat2);
begin
  InitFormat(Format);
...
procedure TIRCTextAttributes.SetAttributes(var Format: TCharFormat2);
begin
  if RichEdit.HandleAllocated then
...
{$IFNDEF VER90}
function TIRCTextAttributes.GetCharset: TFontCharset;
var
  Format: TCharFormat2;
begin
...
procedure TIRCTextAttributes.SetCharset(Value: TFontCharset);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetProtected: Boolean;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetProtected(Value: Boolean);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetLink: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TIRCTextAttributes.SetLink(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetRevAuthorIndex: Byte;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetRevAuthorIndex(Value: Byte);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetHidden: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TIRCTextAttributes.SetHidden(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetDisabled: Boolean;
var
  Format: TCharFormat2;
begin
  Result := False;
...
procedure TIRCTextAttributes.SetDisabled(Value: Boolean);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetColor: TColor;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetColor(Value: TColor);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetBackColor: TColor;
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
procedure TIRCTextAttributes.SetBackColor(Value: TColor);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetName: TFontName;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetName(Value: TFontName);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetStyle: TFontStyles;
var
  Format: TCharFormat2;
begin
  Result := [];
...
procedure TIRCTextAttributes.SetStyle(Value: TFontStyles);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetUnderlineType: TUnderlineType;
var
  Format: TCharFormat2;
begin
  Result := utNone;
...
procedure TIRCTextAttributes.SetUnderlineType(Value: TUnderlineType);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCTextAttributes.GetOffset: Integer;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetOffset(Value: Integer);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetSize: Integer;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetSize(Value: Integer);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetHeight: Integer;
begin
  Result := MulDiv(Size, RichEdit.FScreenLogPixels, 72);
end;

procedure TIRCTextAttributes.SetHeight(Value: Integer);
begin
  Size := MulDiv(Value, 72, RichEdit.FScreenLogPixels);
end;

function TIRCTextAttributes.GetPitch: TFontPitch;
var
  Format: TCharFormat2;
begin
  GetAttributes(Format);
...
procedure TIRCTextAttributes.SetPitch(Value: TFontPitch);
var
  Format: TCharFormat2;
begin
  InitFormat(Format);
...
function TIRCTextAttributes.GetSubscriptStyle: TSubscriptStyle;
var
  Format: TCharFormat2;
begin
  Result := ssNone;
...
procedure TIRCTextAttributes.SetSubscriptStyle(Value: TSubscriptStyle);
var
  Format: TCharFormat2;
begin
  if RichEditVersion < 2 then
...
procedure TIRCTextAttributes.AssignFont(Font: TFont);
var
  LogFont: TLogFont;
  Format: TCharFormat2;
begin
...
procedure TIRCTextAttributes.Assign(Source: TPersistent);
var
  Format: TCharFormat2;
begin
  ...
  else if Source is TIRCTextAttributes then
  begin
    TIRCTextAttributes(Source).GetAttributes(Format);
    SetAttributes(Format);
  end
  else inherited Assign(Source);
end;

procedure TIRCTextAttributes.AssignTo(Dest: TPersistent);
begin
...
constructor TIRCParaAttributes.Create(AOwner: TIRCCustomRichEdit);
begin
  inherited Create;
  RichEdit := AOwner;
end;

procedure TIRCParaAttributes.InitPara(var Paragraph: TParaFormat2);
begin
...
procedure TIRCParaAttributes.GetAttributes(var Paragraph: TParaFormat2);
begin
  InitPara(Paragraph);
  if RichEdit.HandleAllocated then
...
procedure TIRCParaAttributes.SetAttributes(var Paragraph: TParaFormat2);
begin
{$IFDEF RX_D4}
...
function TIRCParaAttributes.GetAlignment: TParaAlignment;
var
...
procedure TIRCParaAttributes.SetAlignment(Value: TParaAlignment);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TIRCParaAttributes.GetNumbering: TIRCNumbering;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
  Result := TIRCNumbering(Paragraph.wNumbering);
  if RichEditVersion = 1 then
    if Result <> nsNone then
      Result := nsBullet;
end;

procedure TIRCParaAttributes.SetNumbering(Value: TIRCNumbering);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion = 1 then
    if Value <> nsNone then
      Value := TIRCNumbering(PFN_BULLET);
  case Value of
    nsNone: LeftIndent := 0;
  else
...
function TIRCParaAttributes.GetNumberingStyle: TIRCNumberingStyle;
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
  ...
  begin
    GetAttributes(Paragraph);
    Result := TIRCNumberingStyle(Paragraph.wNumberingStyle);
  end;
end;

procedure TIRCParaAttributes.SetNumberingStyle(Value: TIRCNumberingStyle);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetNumberingTab: Word;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetNumberingTab(Value: Word);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetFirstIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetFirstIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TIRCParaAttributes.GetHeadingStyle: THeadingStyle;
var
  Paragraph: TParaFormat2;
begin
...
procedure TIRCParaAttributes.SetHeadingStyle(Value: THeadingStyle);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 3 then
...
function TIRCParaAttributes.GetLeftIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetLeftIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TIRCParaAttributes.GetRightIndent: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetRightIndent(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  InitPara(Paragraph);
...
function TIRCParaAttributes.GetSpaceAfter: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetSpaceAfter(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetSpaceBefore: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetSpaceBefore(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetLineSpacing: Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetLineSpacing(Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetLineSpacingRule: TLineSpacingRule;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetLineSpacingRule(Value: TLineSpacingRule);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
function TIRCParaAttributes.GetTab(Index: Byte): Longint;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetTab(Index: Byte; Value: Longint);
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
function TIRCParaAttributes.GetTabCount: Integer;
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
procedure TIRCParaAttributes.SetTabCount(Value: Integer);
var
  Paragraph: TParaFormat2;
begin
  GetAttributes(Paragraph);
...
function TIRCParaAttributes.GetTableStyle: TParaTableStyle;
var
  Paragraph: TParaFormat2;
begin
  Result := tsNone;
...
procedure TIRCParaAttributes.SetTableStyle(Value: TParaTableStyle);
var
  Paragraph: TParaFormat2;
begin
  if RichEditVersion < 2 then
...
procedure TIRCParaAttributes.AssignTo(Dest: TPersistent);
var
  I: Integer;
begin
...
procedure TIRCParaAttributes.Assign(Source: TPersistent);
var
  I: Integer;
  Paragraph: TParaFormat2;
begin
  ...
  begin
    ...
    Numbering := TIRCNumbering(TParaAttributes(Source).Numbering);
    for I := 0 to MAX_TAB_STOPS - 1 do
      Tab[I] := TParaAttributes(Source).Tab[I];
  end
  else if Source is TIRCParaAttributes then
  begin
    TIRCParaAttributes(Source).GetAttributes(Paragraph);
    SetAttributes(Paragraph);
  end
  else inherited Assign(Source);
end;
...
begin
  ...
  try
    OleCheck(StgCreateDocfileOnILockBytes(LockBytes, STGM_READWRITE or
      STGM_SHARE_EXCLUSIVE or STGM_CREATE, 0, Storage));
  finally
...
{$ENDIF}
  private
    FRefCount: Longint;
    FRichEdit: TIRCCustomRichEdit;
  public
    constructor Create(RichEdit: TIRCCustomRichEdit);
    destructor Destroy; override;
{$IFDEF RX_D3}
...
constructor TRichEditOleCallback.Create(RichEdit: TIRCCustomRichEdit);
begin
  inherited Create;
  FRichEdit := RichEdit;
end;
...
function TRichEditOleCallback.GetClipboardData(const chrg: TCharRange;
  reco: DWORD; {$IFDEF RX_D3} out {$ELSE} var {$ENDIF} dataobj: IDataObject): HResult;
begin
  Result := E_NOTIMPL;
end;
...
{$ENDIF}
  private
    ...
    FReObject: TReObject;
  public
    constructor Create(RichEdit: TIRCCustomRichEdit; ReObject: TReObject);
  {$IFNDEF RX_D3}
    function QueryInterface(const iid: TIID; var obj): HResult; override;
    function AddRef: Longint; override;
    function Release: Longint; override;
...
constructor TOleUIObjInfo.Create(RichEdit: TIRCCustomRichEdit;
  ReObject: TReObject);
begin
  inherited Create;
  FRichEdit := RichEdit;
...
constructor TOleUILinkInfo.Create(RichEdit: TIRCCustomRichEdit;
  ReObject: TReObject);
begin
  inherited Create;
  FReObject := ReObject;
...
function TOleUILinkInfo.SetLinkSource(dwLink: Longint; pszDisplayName: PChar;
  lenFileName: Longint; var chEaten: Longint; fValidateSource: BOOL): HResult;
var
  DisplayName: String;
...
function TOleUILinkInfo.GetLinkSource(dwLink: Longint;
  var pszDisplayName: PChar; var lenFileName: Longint;
  var pszFullLinkType: PChar; var pszShortLinkType: PChar;
  var fSourceAvailable: BOOL; var fIsSelected: BOOL): HResult;
var
  Moniker: IMoniker;
begin
...
constructor TIRCCustomRichEdit.Create(AOwner: TComponent);
var
  DC: HDC;
begin
  inherited Create(AOwner);
  ControlStyle := ControlStyle - [csSetCaption];
  FSelAttributes := TIRCTextAttributes.Create(Self, atSelected);
  FDefAttributes := TIRCTextAttributes.Create(Self, atDefaultText);
  FWordAttributes := TIRCTextAttributes.Create(Self, atWord);
  FParagraph := TIRCParaAttributes.Create(Self);
...
destructor TIRCCustomRichEdit.Destroy;
begin
  FLastFind := nil;
  FSelAttributes.Free;
  FDefAttributes.Free;
...
procedure TIRCCustomRichEdit.Clear;
begin
  CloseObjects;
  inherited Clear;
  Modified := False;
...
procedure TIRCCustomRichEdit.CreateParams(var Params: TCreateParams);
const
...
procedure TIRCCustomRichEdit.CreateWnd;
var
...
procedure TIRCCustomRichEdit.DestroyWnd;
var
  StreamFmt: TRichStreamFormat;
  Mode: TRichStreamModes;
  DesignMode: Boolean;
...
procedure TIRCCustomRichEdit.SetAllowObjects(Value: Boolean);
begin
...
procedure TIRCCustomRichEdit.UpdateHostNames;
var
  AppName: String;
begin
...
procedure TIRCCustomRichEdit.SetTitle(const Value: String);
begin
...
function TIRCCustomRichEdit.GetPopupMenu: TPopupMenu;
var
...
procedure TIRCCustomRichEdit.PopupVerbClick(Sender: TObject);
var
  ReObject: TReObject;
begin
...
procedure TIRCCustomRichEdit.ObjectPropsClick(Sender: TObject);
begin
  ObjectPropertiesDialog;
end;

procedure TIRCCustomRichEdit.WMSetFont(var Message: TWMSetFont);
begin
  FDefAttributes.Assign(Font);
end;

procedure TIRCCustomRichEdit.CMFontChanged(var Message: TMessage);
begin
  inherited;
  FDefAttributes.Assign(Font);
end;

procedure TIRCCustomRichEdit.CreateWindowHandle(const Params: TCreateParams);
var
  Bounds: TRect;
begin
  Bounds := BoundsRect;
...
{$IFDEF RX_D3}
procedure TIRCCustomRichEdit.DoSetMaxLength(Value: Integer);
begin
...
function TIRCCustomRichEdit.GetCaretPos: TPoint;
var
...
{$IFDEF RX_D3}
function TIRCCustomRichEdit.GetSelLength: Integer;
begin
  with GetSelection do
    Result := cpMax - cpMin;
end;

function TIRCCustomRichEdit.GetSelStart: Integer;
begin
  Result := GetSelection.cpMin;
end;

function TIRCCustomRichEdit.GetSelText: String;
begin
  with GetSelection do
...
function TIRCCustomRichEdit.GetSelTextBuf(Buffer: PChar; BufSize: Integer): Integer;
var
  S: String;
begin
  S := SelText;
...
{$IFDEF RX_D4}
procedure TIRCCustomRichEdit.CMBiDiModeChanged(var Message: TMessage);
var
...
procedure TIRCCustomRichEdit.SetHideScrollBars(Value: Boolean);
begin
...
procedure TIRCCustomRichEdit.SetSelectionBar(Value: Boolean);
begin
...
procedure TIRCCustomRichEdit.SetHideSelection(Value: Boolean);
begin
...
function TIRCCustomRichEdit.GetAutoURLDetect: Boolean;
begin
...
procedure TIRCCustomRichEdit.SetAutoURLDetect(Value: Boolean);
begin
...
function TIRCCustomRichEdit.GetWordSelection: Boolean;
begin
...
procedure TIRCCustomRichEdit.SetWordSelection(Value: Boolean);
var
  Options: LPARAM;
begin
...
function TIRCCustomRichEdit.GetLangOptions: TRichLangOptions;
var
  Flags: Longint;
  I: TRichLangOption;
begin
...
procedure TIRCCustomRichEdit.SetLangOptions(Value: TRichLangOptions);
var
  Flags: DWORD;
  I: TRichLangOption;
begin
...
procedure TIRCCustomRichEdit.SetSelAttributes(Value: TIRCTextAttributes);
begin
  FSelAttributes.Assign(Value);
end;

function TIRCCustomRichEdit.GetCanRedo: Boolean;
begin
  Result := False;
  if HandleAllocated and (RichEditVersion >= 2) then
...
function TIRCCustomRichEdit.GetCanPaste: Boolean;
begin
  Result := False;
  if HandleAllocated then
...
{$IFNDEF RX_V110}
function TIRCCustomRichEdit.GetCanUndo: Boolean;
begin
  Result := False;
  if HandleAllocated then
...
function TIRCCustomRichEdit.GetRedoName: TUndoName;
begin
  Result := unUnknown;
...
function TIRCCustomRichEdit.GetUndoName: TUndoName;
begin
  Result := unUnknown;
  if (RichEditVersion >= 2) and HandleAllocated then
...
function TIRCCustomRichEdit.GetSelectionType: TRichSelectionType;
const
...
function TIRCCustomRichEdit.GetSelection: TCharRange;
begin
  SendMessage(Handle, EM_EXGETSEL, 0, Longint(@Result));
end;

procedure TIRCCustomRichEdit.SetSelection(StartPos, EndPos: Longint;
  ScrollCaret: Boolean);
var
  CharRange: TCharRange;
begin
...
{$IFDEF RX_D3}
procedure TIRCCustomRichEdit.SetSelLength(Value: Integer);
begin
  with GetSelection do
    SetSelection(cpMin, cpMin + Value, True);
end;

procedure TIRCCustomRichEdit.SetSelStart(Value: Integer);
begin
  SetSelection(Value, Value, False);
end;
{$ENDIF RX_D3}

function TIRCCustomRichEdit.GetCharPos(CharIndex: Integer): TPoint;
var
  Res: Longint;
begin
...
function TIRCCustomRichEdit.GetTextRange(StartPos, EndPos: Longint): String;
var
...
function TIRCCustomRichEdit.WordAtCursor: String;
var
  Range: TCharRange;
begin
  Result := '';
...
function TIRCCustomRichEdit.LineFromChar(CharIndex: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_EXLINEFROMCHAR, 0, CharIndex);
end;

function TIRCCustomRichEdit.GetLineIndex(LineNo: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_LINEINDEX, LineNo, 0);
end;

function TIRCCustomRichEdit.GetLineLength(CharIndex: Integer): Integer;
begin
  Result := SendMessage(Handle, EM_LINELENGTH, CharIndex, 0);
end;

procedure TIRCCustomRichEdit.SetUndoLimit(Value: Integer);
begin
...
procedure TIRCCustomRichEdit.SetDefAttributes(Value: TIRCTextAttributes);
begin
  FDefAttributes.Assign(Value);
end;

procedure TIRCCustomRichEdit.SetWordAttributes(Value: TIRCTextAttributes);
begin
  FWordAttributes.Assign(Value);
end;

function TIRCCustomRichEdit.GetStreamFormat: TRichStreamFormat;
begin
  Result := TRichEditStrings(Lines).Format;
end;

function TIRCCustomRichEdit.GetStreamMode: TRichStreamModes;
begin
  Result := TRichEditStrings(Lines).Mode;
end;

procedure TIRCCustomRichEdit.SetStreamFormat(Value: TRichStreamFormat);
begin
  TRichEditStrings(Lines).Format := Value;
end;

procedure TIRCCustomRichEdit.SetStreamMode(Value: TRichStreamModes);
begin
  TRichEditStrings(Lines).Mode := Value;
end;

procedure TIRCCustomRichEdit.SetPlainText(Value: Boolean);
var
  MemStream: TStream;
  StreamFmt: TRichStreamFormat;
  Mode: TRichStreamModes;
...
procedure TIRCCustomRichEdit.UpdateTextModes(Plain: Boolean);
const
...
procedure TIRCCustomRichEdit.CMColorChanged(var Message: TMessage);
begin
  inherited;
  SendMessage(Handle, EM_SETBKGNDCOLOR, 0, ColorToRGB(Color))
end;

procedure TIRCCustomRichEdit.EMReplaceSel(var Message: TMessage);
var
  CharRange: TCharRange;
begin
...
procedure TIRCCustomRichEdit.SetRichEditStrings(Value: TStrings);
begin
  FRichEditStrings.Assign(Value);
end;

procedure TIRCCustomRichEdit.CloseObjects;
var
  I: Integer;
  ReObject: TReObject;
begin
...
function TIRCCustomRichEdit.PasteSpecialDialog: Boolean;
...
function TIRCCustomRichEdit.InsertObjectDialog: Boolean;
var
  ...
  begin
    ...
    try
      ...
      try
        ...
        if Result then
        try
          IsNewObject := Data.dwFlags and
            IOF_SELECTCREATENEW = IOF_SELECTCREATENEW;
          with ReObject do
          begin
            cbStruct := SizeOf(TReObject);
            cp := REO_CP_SELECTION;
          ...
          IRichEditOle(FRichEditOle).SetDvaspect(
            Longint(REO_IOB_SELECTION), ReObject.dvAspect);
          if IsNewObject then
            OleObject.DoVerb(OLEIVERB_SHOW, nil, OleClientSite, 0, Handle,
              ClientRect);
        finally
          ReleaseObject(OleObject);
        end;
...
function TIRCCustomRichEdit.ObjectPropertiesDialog: Boolean;
var
...
procedure TIRCCustomRichEdit.Print(const Caption: String);
var
  Range: TFormatRange;
...
procedure TIRCCustomRichEdit.WMPaint(var Message: TWMPaint);
var
  R, R1: TRect;
begin
  if RichEditVersion >= 2 then
...
procedure TIRCCustomRichEdit.WMDestroy(var Msg: TWMDestroy);
begin
  CloseObjects;
...
procedure TIRCCustomRichEdit.WMMouseMove(var Message: TMessage);
begin
  inherited;
end;

procedure TIRCCustomRichEdit.WMSetCursor(var Message: TWMSetCursor);
begin
  inherited;
end;

{$IFDEF RX_D5}
procedure TIRCCustomRichEdit.WMRButtonUp(var Message: TMessage);
begin
...
procedure TIRCCustomRichEdit.CNNotify(var Message: TWMNotify);
var
  AMsg: TMessage;
begin
  with Message do
...
function TIRCCustomRichEdit.SaveClipboard(NumObj, NumChars: Integer): Boolean;
begin
  Result := True;
...
function TIRCCustomRichEdit.ProtectChange(const Message: TMessage; StartPos,
...
procedure TIRCCustomRichEdit.SelectionChange;
begin
  if Assigned(OnSelectionChange) then
    OnSelectionChange(Self);
end;

procedure TIRCCustomRichEdit.RequestSize(const Rect: TRect);
begin
  if Assigned(OnResizeRequest) then
    OnResizeRequest(Self, Rect);
end;

procedure TIRCCustomRichEdit.URLClick(const URLText: String;
  Button: TMouseButton);
begin
  ShellExecute(Application.Handle, 'open', PChar(URLText), nil, nil, 0);
  if Assigned(OnURLClick) then
    OnURLClick(Self, URLText, Button);
end;

function GetSpechialCharPos(mes: AnsiString; var _type: Char): Integer;
var
  _bold, _underline, _neg, _color, _simply: Integer;
begin
  _bold := Pos(#2, mes);
  _underline := Pos(#31, mes);
  _neg := Pos(#22, mes);
  _color := Pos(#3, mes);
  _simply := Pos(#15, mes);
  Result := 0;
  if ((_bold < result) or (result = 0)) and (_bold > 0) then
  begin
    _type := 'b';
    Result := _bold;
  end;
  if ((_underline < Result) or (Result = 0)) and (_underline > 0) then
  begin
    _type := 'u';
    Result := _underline;
  end;
  if ((_neg < Result) or (Result = 0)) and (_neg > 0) then
  begin
    _type := 'n';
    Result := _neg;
  end;
  if ((_color < Result) or (Result = 0)) and (_color > 0) then
  begin
    _type := 'c';
    Result := _color;
  end;
  if ((_simply < Result) or (Result = 0)) and (_simply > 0) then
  begin
    _type := 's';
    Result := _simply;
  end;
...
function StrDelete(SubStr, S: String; var Deleted: Integer): String;
var
  P: Integer;
  L: Integer;
begin
  ...
  begin
    Deleted := Deleted + 1;
    //Delete(S, P, L);
    S := Copy(S, 1, P - 1) + ' ' + Copy(S, P + L, Length(S) - L);
    P := Pos(SubStr, S);
  end;
  Result := S;
end;

function CountNewLines(S: AnsiString): Integer;
var
  L, I: Integer;
begin
  Result := 0;
...
type
  TFormated = array[0..100] of record
    _start: Integer;
    _type: Char;
    color, bgcolor: TColor;
  end;

procedure GetFormatedPositions(var Mess: String; var _Formated: TFormated;
  var Count_Formated: Integer);
var
  P1, P2: Integer;
  TypeText, NewTypeText: Char;
  CS: Char;
  ColorNumber: Integer;
  ColorNumberStr: String[3];
  Code: Integer;
  bgColor: TColor;
begin
  bgColor := clWhite;
  // Определяем где должна быть цветовая разметка
  P1 := GetSpechialCharPos(Mess, TypeText);
  Count_Formated := 0;
  while P1 > 0 do
  begin
    ...
    begin
      ...
      _Formated[Count_Formated].color := clBlack;
      _Formated[Count_Formated].bgcolor := bgColor;
      ...
      begin
        bgColor := clWhite;
        _Formated[Count_Formated].bgcolor := clWhite;
      end;

      // Если задали цвет фона
      if ({Copy(Mess, P1, 1)}Mess[P1] = ',') then
      begin
        Delete(Mess, P1, 1);
        ColorNumberStr := '';
        CS := {Copy(Mess, P1, 1)}Mess[P1];
    ...
    P1 := P1 - 1;
    P2 := GetSpechialCharPos(Mess, NewTypeText);
    _Formated[Count_Formated]._start := P1;
    _Formated[Count_Formated]._type := TypeText;
    P1 := P2;
    TypeText := NewTypeText;
    Count_Formated := Count_Formated + 1;
  end;
end;

function TIRCCustomRichEdit.FindText(const SearchStr: String;
  StartPos, Length: Integer; Options: TRichSearchTypes): Integer;
var
  Find: TFindTextEx;
  Flags: Integer;
...
procedure TIRCCustomRichEdit.ClearUndo;
begin
  SendMessage(Handle, EM_EMPTYUNDOBUFFER, 0, 0);
end;

procedure TIRCCustomRichEdit.Redo;
begin
  SendMessage(Handle, EM_REDO, 0, 0);
end;

{$IFNDEF RX_V110}
procedure TIRCCustomRichEdit.Undo;
begin
  SendMessage(Handle, WM_UNDO, 0, 0);
end;
{$ENDIF}

procedure TIRCCustomRichEdit.StopGroupTyping;
begin
  if (RichEditVersion >= 2) and HandleAllocated then
...
{$IFDEF RX_D3}
procedure TIRCCustomRichEdit.SetUIActive(Active: Boolean);
var
  Form: TCustomForm;
begin
  try
...
procedure TIRCCustomRichEdit.CMDocWindowActivate(var Message: TMessage);
begin
...
procedure TIRCCustomRichEdit.CMUIDeactivate(var Message: TMessage);
begin
...
procedure TIRCCustomRichEdit.SetupFindDialog(Dialog: TFindDialog;
...
function TIRCCustomRichEdit.FindDialog(const SearchStr: String): TFindDialog;
begin
...
function TIRCCustomRichEdit.ReplaceDialog(const SearchStr,
...
function TIRCCustomRichEdit.GetCanFindNext: Boolean;
begin
  Result := HandleAllocated and (FLastFind <> nil) and
    (FLastFind.FindText <> '');
end;

function TIRCCustomRichEdit.FindNext: Boolean;
begin
  if CanFindNext then
...
procedure TIRCCustomRichEdit.AdjustFindDialogPosition(Dialog: TFindDialog);
var
  TextRect, R: TRect;
begin
...
function TIRCCustomRichEdit.FindEditText(Dialog: TFindDialog; AdjustPos,
  Events: Boolean): Boolean;
var
...
procedure TIRCCustomRichEdit.TextNotFound(Dialog: TFindDialog);
begin
  if Assigned(FOnTextNotFound) then
    FOnTextNotFound(Self, Dialog.FindText);
end;

procedure TIRCCustomRichEdit.FindDialogFind(Sender: TObject);
begin
  FindEditText(TFindDialog(Sender), True, True);
end;

procedure TIRCCustomRichEdit.ReplaceDialogReplace(Sender: TObject);
var
  Cnt: Integer;
...
{$IFDEF RX_D3}
procedure TIRCCustomRichEdit.FindDialogClose(Sender: TObject);
begin
  CloseFindDialog(Sender as TFindDialog);
end;

procedure TIRCCustomRichEdit.CloseFindDialog(Dialog: TFindDialog);
begin
...
class procedure TIRCCustomRichEdit.RegisterConversionFormat(
  const AExtension: String; APlainText: Boolean;
...
procedure TIRCRichEdit.SetSmiles(Value:TImageList);
begin
  FSmiles := Value;
end;

procedure TIRCRichEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
begin
  ...
  while FIRCRichEditBusy do
    Sleep(100);

  FIRCRichEditBusy := True;
  SelAttrsDefault;
  ...
  begin
    //Application.ProcessMessages;
    NoSmilesMess := StrDelete(SmilesInfo[I].Smile, NoSmilesMess, Deleted);
    CountSmiles := CountSmiles + Deleted;
  end;

  GetFormatedPositions(Mess, _Formated, Count_Formated);
  FillChar(_Formated, Sizeof(_Formated), 0);
  GetFormatedPositions(NoSmilesMess, _Formated, Count_Formated);

  // Выводим смайлы
  // сдвигаем курсор на конец текста
  SelStart := Length(Lines.Text);
  SelLength := 0;

  // Устанавливаем значение смайла №3 -
  // просто так чтобы цикл начал работу
  Smil_Num := 3;

  // Цикл, в котором будет определяться положение
  // ближайшего смайла от начала текста и выводиться кусок текста сразу
  // до этого смайла, затем будет выводиться сам смайл и по новой...
  Mess2 := '';

  while (Smil_Num <> -1) do
  begin
    //Application.ProcessMessages;
    // Устанавливаем позицию смайла в конец текста и обнуляем его значение
    Smil_Pos := Length(Mess) + 1;
    Smil_Num := -1;
    // В цикле регистронезависимо определяем позицию ближайшего смайла
    for L := 0 to MAX_SMILES_INFO - 1 do
    begin
      //Application.ProcessMessages;
      if (SmilesInfo[L].Smile <> '') then
      begin
        P := Pos(SmilesInfo[L].Smile, Mess);
        if (P < Smil_Pos) and (P <> 0) then
        begin
          Smil_Pos := P;
          Smil_Num := L;
        end;
    ...
    // Выводит текст до смайла
    SetSelTextBuf(PChar(Copy(Mess, 1, Smil_Pos - 1)));
    Mess2 := Mess2 + Copy(Mess, 1, Smil_Pos - 1);

    // Удаляем из переменной сообщения выведенный текст и сам смайл
    Delete(Mess, 1, Smil_Pos + (Length(SmilesInfo[Smil_Num].Smile) - 1));

    // Сдвигаем курсор на конец текста
    SelStart := Length(Self.Lines.Text);
    SelLength := 0;

    // Если позиция смайла не нулевая (то есть смайл действительно нужно вывести)
    // то попросту выводим из массива rtf последовательность, согласно номеру определённого смайла
    if (Smil_Num <> -1) then
    begin
      if (SmilesInfo[Smil_Num].rtf = '') then
      begin
        Bitmap := TBitmap.Create;
        FSmiles.GetBitmap(SmilesInfo[Smil_Num].ImageIndex, Bitmap);
        //SmilesInfo[smil_num].Bitmap.SaveToFile('smil_num'+IntToStr(smil_num)+'.bmp');
        BitmapToRTF(Bitmap, Anss);
        SmilesInfo[Smil_Num].rtf := StrPas(PChar(Anss));
      end;
      SS := TStringStream.Create(SmilesInfo[Smil_Num].rtf);
  ...
  SetSelTextBuf(PChar(Chr(13) + Chr(10)));
  MessLength := Length(Mess2);

  // Делаем разметку
  if (Count_Formated > 0) then
  begin
    AllLength := Length(Self.Text) - MessLength - CountNewLines(Self.Text) -
      CountSmiles - 1;
    for I := 0 to Count_Formated - 1 do
    begin
      //Application.ProcessMessages;
      SelStart := AllLength + _Formated[I]._start;
      SelLength := Length(Self.Text);
      case _Formated[I]._type of
        'b':
          begin
            if not FlgBold then
            begin
        ...
        'n':
          begin
            if not FlgInvert then
            begin
              FlgInvert := True;
              SelAttributes.Color := clWhite;
              SelAttributes.BackColor := clBlack;
            end
            else
            begin
              FlgInvert := False;
        ...
        'c':
          begin
            //if (_Formated[i].color = clBlack) and (_Formated[i].bgcolor = clWhite) then
            SelAttributes.Color := _Formated[I].color;
            SelAttributes.BackColor := _Formated[I].bgcolor;
            //_Formated[I].color := IRCPalette[StrToInt()];
          end;
        's':
          begin
            SelAttributes.Color := clBlack;
...
procedure TIRCRichEdit.SelAttrsDefault;
begin
