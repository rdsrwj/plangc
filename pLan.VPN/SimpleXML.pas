{************************************************************
 ...
 (с) Авторские права 2002 Михаил Власов.
...
function CreateXmlDocument(
	...
	const anEncoding: String = 'windows-1251';
...
function DateTimeToXSTR(v: TDateTime): TXmlString;
begin
	Result := FormatDateTime('yyyy-mm-ddThh-nn-ss', v)
end;
...
begin
	case v.VType of
		...
		varBoolean: Result := BoolStr[v.VBoolean];
...
type
	...
	private
		...
		function GetChilds: TXmlNodeList;
	...
	protected
		...
		function CreateComment(const aData: TXmlString): IXmlComment;
		function Get_DocumentElement: IXmlElement;
		function Get_OwnerDocument: IXmlDocument; override;
		function CreateProcessingInstruction(const aTarget,
			aData: TXmlString): IXmlProcessingInstruction; overload;
		function CreateProcessingInstruction(aTargetID: Integer;
			const aData: TXmlString): IXmlProcessingInstruction; overload;
...
procedure TXmlNodeList.ParseXML(aXML: TXmlSource; aNames: TXmlNameTable);
	...
	begin
		...
		if aText^ <> #0 then begin
			aStr := aText;
			Insert(TXmlText.Create(aNames, aStr), -1);
		end
	end;
	...
	begin
		...
		if aXml.EOF then
			raise Exception.Create('Не завершено определение элемента');
		if not ((aXml.CurChar <= ' ') or (aXml.CurChar = '/') or (aXml.CurChar = '>')) then
			raise Exception.Create('Некорретный символ в имени элемента');
		...
		if aXml.CurChar = '/' then 
			...
			aXml.ExpectChar('>');
			aNode.GetChilds.ParseXML(aXml, aNames);
			aXml.ExpectChar('/');
	...
	for i := 0 to aCount - 1 do begin
		...
		case aNodeType of
			...
			NODE_COMMENT:
				...
				raise Exception.Create('Ошибка чтения двоичного XML: некорректный тип узла');
		end
	end;
end;
	...
	for i := 0 to aCount - 1 do begin
		...
		case aNodeType of
			...
			NODE_COMMENT:
				...
				raise Exception.Create('Ошибка записи двоичного XML: некорректный тип узла');
		end
	end;
end;
...
begin
	...
	raise Exception.CreateFmt(
		'Неверное значение атрибута "%s" элемента "%s".'^M^J +
		'Допустимые значения:'^M^J +
		'%s', [aNode.FNames.GetName(anAttrData.NameID), aNode.Get_NodeName, s]);
end;
...
begin
	...
	if Assigned(anAttrData) then
		...
		raise Exception.Create('Не найден атрибут "' + FNames.GetName(aNameID) + '"');
end;
...
begin
	...
	if not Assigned(anAttr) then
		raise Exception('Не задан атрибут "' + FNames.GetName(aNameID) + '"');
	Result := anAttr.Value
end;
...
begin
	...
	while Assigned(aParent) do begin
		Result := aParent.Get_OwnerDocument;
...
begin
	if IsName(anExpression) then begin
		...
		raise
			Exception.Create('Ансаппортед ИксПуть экспрешшэн');
{
...
begin
	if Assigned(aNames) then
		...
		aNameTable := DefaultNameTable.GetObject as TXmlNameTable;
...
function TXmlDocument.Get_OwnerDocument: IXmlDocument;
begin
	Result := Self
end;
...
begin
	...
	try
		GetChilds.ParseXML(aXml, FNames);
	finally
		aXml.Free
	end
end;
...
begin
	if XmlIsInBinaryFormat(aXML) then
		...
		try
			GetChilds.ParseXML(aSource, FNames);
		finally
			aSource.Free
		end
	end
...
begin
	...
	aChilds.Insert(TXmlProcessingInstruction.Create(FNames, FNames.FXmlID,
		'version="' + aVersion + '" encoding="' + anEncoding + '"'), 0);
...
function TXmlSource.AcceptToken: PXmlChar;
begin
	FToken.FValuePtr^ := #0;
	Result := FToken.ValueStart;
	DropToken;
end;
...
begin
	if not NameCanBeginWith(CurChar) then
		raise Exception.Create('Имя должно начинаться с буквы или "_"');
	NewToken;
...
begin
	...
	s := AcceptToken;
	if Length(s) = 0 then
		raise Exception.Create('Ожидается число');
	Val(s, Result, e);
end;
...
begin
	...
	s := s + AcceptToken;
	if Length(s) = 1 then
		raise Exception.Create('Ожидается шестнадцатеричное число');
	Val(s, Result, e);
end;
...
begin
	if not Next then
		raise Exception.Create('Ожидается "#" или имя упрамляющего символа');
	if CurChar = '#' then begin
		...
		if s = 'amp' then
			...
			raise Exception.Create('Некорректное имя управляющего символа');
	end
end;
...
begin
	if EOF or (CurChar <> aChar) then
		raise Exception.Create('Ожидается "' + aChar + '"');
	Next;
end;
...
begin
	while aText^ <> #0 do begin
		if (CurChar <> aText^) or EOF then
			raise Exception.Create('Ожидается "' + aText + '"');
		Inc(aText);
		Next;
	end;
end;
...
begin
	...
	while not EOF and (CurChar <> aQuote) do begin
		if CurChar = '&' then
			...
			raise Exception.Create('Символ "<" не может использоваться в значениях атрибутов')
		else begin
			AppendTokenChar(CurChar);
			Next;
		end
	...
	if EOF then
		raise Exception.Create('Ожидается "' + aQuote + '"');
	Next;
	Result := AcceptToken;
end;
...
begin
	...
	while not EOF and NameCanBeginWith(CurChar) do begin
		...
		SkipBlanks;
		if EOF then
			raise Exception.Create('Ожидается значение атрибута');
		if (CurChar = '''') or (CurChar = '"') then
			aValue := ExpectQuotedText(CurChar)
		else
			raise Exception.Create('Ожидается строковая константа');
...
begin
	if aSize > FRestSize then
		raise Exception.Create('Ошибка чтения данных.');
...
begin
	...
	case aDataType of
		...
		varArray + varByte:
			...
			raise Exception.Create('Ошибка чтения значения: некорректный тип.');
	end;
	v.VType := aDataType;
end;
...
begin
	...
	case v.VType of
		...
		varArray + varByte:
			...
			raise Exception.Create('Ошибка записи значения: некорректный тип.');
	end;
end;
...
initialization
	DefaultNameTable := CreateNameTable;
finalization
