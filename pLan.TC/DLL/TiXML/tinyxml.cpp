...
int TiXmlElement::QueryFloatAttribute( const char* name, float* dval ) const
{
	double dummy(static_cast<double>(*dval));
	int retVal = QueryDoubleAttribute(name, &dummy);
	*dval = static_cast<float>(dummy);
	return retVal;
}
